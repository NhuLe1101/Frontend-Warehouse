...
const PopupItems = ({ compartmentData, onClose, isItemPresent }) => {
  ...
  const handleConfirmCheckout = (referenceNo, plateNumber) => {
    ...
    CompartmentService.checkoutItem(
      compartmentId,
      itemId,
      referenceNo,
      plateNumber
      ...
      .catch((error) => {
        setSnackbarMessage(
          error.response?.data.message ||
            "Có lỗi xảy ra trong quá trình checkout"
        );
        setSnackbarSeverity("error");
        setOpenSnackbar(true);
      });
    ...
    <div className="popup-items">
      ...
      {isItemPresent ? (
        ...
        <>
          ...
          <h2>
            Chọn sản phẩm để thêm vào ngăn {compartmentData.nameComp} kệ{" "}
            {compartmentData.shelf.nameShelf}
          </h2>
      ...
      <Dialog open={openDeleteDialog} onClose={cancelDelete}>
        ...
        <DialogContent>
          <DialogContentText>
            Bạn có chắc chắn muốn xóa sản phẩm này khỏi ngăn chứa không? Hành
